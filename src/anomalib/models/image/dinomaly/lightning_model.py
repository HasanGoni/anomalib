...
class Dinomaly(AnomalyModule):
    ...
    def configure_transforms(image_size: tuple[int, int] | None = None) -> Transform:
        """Configure the default transforms for Dinomaly.
        ...
        Returns:
            Transform: Configured transform for Dinomaly.
    ...
    def training_step(self, batch: dict[str, str | torch.Tensor], *args, **kwargs) -> STEP_OUTPUT:
        ...
        Args:
            batch (dict[str, str | torch.Tensor]): Input batch containing images and metadata.
    ...
    def validation_step(self, batch: dict[str, str | torch.Tensor], *args, **kwargs) -> STEP_OUTPUT:
        ...
        Returns:
            STEP_OUTPUT: Updated batch with pred_scores (anomaly scores) and
                anomaly_maps (pixel-level anomaly maps) predictions.
        """
        del args, kwargs  # These variables are not used.
